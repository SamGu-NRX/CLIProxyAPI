...
func (s *SessionAffinitySelector) pickLCP(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, auths []*Auth, entry *log.Entry) (*Auth, bool, error) {
	...
	if match, ok := s.matcher.MatchFingerprints(namespace, fingerprints, minPrefixLength); ok {
		...
		for _, auth := range available {
			...
			if s.fallbackCache != nil && match.SessionID != "" {
				s.fallbackCache.Invalidate(lcpFallbackCacheKey(namespace, match.SessionID))
			}
		...
		if s.fallbackCache != nil && match.SessionID != "" {
			if fallbackAuthID, okFallback := s.fallbackCache.GetAndRefresh(lcpFallbackCacheKey(namespace, match.SessionID)); okFallback {
				for _, auth := range available {
		...
		if s.fallbackCache != nil && match.SessionID != "" {
			s.fallbackCache.Set(lcpFallbackCacheKey(namespace, match.SessionID), auth.ID)
		}
...
func (s *SessionAffinitySelector) OnResult(res Result) {
	...
	// LCP bindings are independent from explicit harness bindings. A temporary
	// fallback must not replace the sticky primary while that primary cools down.
	if explicitID == "" && s.matcher != nil {
		if namespace := lcpAffinityNamespace(ns, nsModel, res.Options.Metadata); namespace != "" {
			...
			if len(fingerprints) > 0 && minPrefixLength > 0 && minPrefixLength <= len(fingerprints) {
				...
				fallbackKey := ""
				if matched && match.SessionID != "" {
					fallbackKey = lcpFallbackCacheKey(namespace, match.SessionID)
				}
				if res.Success {
					if matched && match.AuthID != res.AuthID {
						if s.fallbackCache != nil && fallbackKey != "" {
							s.fallbackCache.Touch(fallbackKey, res.AuthID)
						}
					} else {
						s.matcher.TouchFingerprints(namespace, fingerprints, minPrefixLength, res.AuthID)
						if s.fallbackCache != nil && fallbackKey != "" {
							s.fallbackCache.Invalidate(fallbackKey)
						}
					}
				} else if res.Error != nil && isTerminalSessionAffinityError(res.Error) {
					if matched && match.AuthID != res.AuthID {
						if s.fallbackCache != nil && fallbackKey != "" {
							s.fallbackCache.CompareAndDelete(fallbackKey, res.AuthID)
						}
					} else {
